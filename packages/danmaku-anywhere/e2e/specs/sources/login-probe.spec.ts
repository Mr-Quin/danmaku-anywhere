...
/**
 * Verifies the generic loginProbe flow end-to-end: opening /providers
 * triggers useProviderWarning per row, which RPCs into the manifest
 * pipeline via providerProbeLogin. Probes return boolean (true = ok,
 * false = needs cookies). Bilibili + tencent are the only manifests
 * with loginProbe today; both are exercised here as exemplars of the
 * generic contract.
 */
...
test('hides warnings when both probes return logged-in / cookies-present', async ({
  ...
  await expect(page.getByTestId('provider-warning')).toHaveCount(0)
})
...
test('surfaces warning icons when probes report logged-out / no cookies', async ({
  ...
  // One per builtin with a loginProbe (bilibili + tencent).
  await expect(page.getByTestId('provider-warning')).toHaveCount(2)
})
