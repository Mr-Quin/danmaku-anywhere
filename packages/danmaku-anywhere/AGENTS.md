...
- **A boolean `define` behaves differently under `vite build` vs vitest/`vite dev`.** An entry like `'import.meta.env.VITE_FOO': JSON.stringify(true)` inlines to a real `true`/`false` literal in a production build, but under vitest and the dev server it surfaces as a truthy string (`"true"`/`"false"`) instead. Encode flags read via `import.meta.env` as strings (`JSON.stringify('true')`/`JSON.stringify('false')`) and type them as a required string union in `vite-env.d.ts`, not `boolean`, so the same comparison holds in both modes.
