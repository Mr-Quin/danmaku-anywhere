...
interface ParseTabCoreProps {
  /** Optional callback called after successful parse and import */
  onImportSuccess?: (episodeData: any) => void
  /** Optional callback called when parse fails */
  onParseError?: (error: Error) => void
  /** Optional callback called when import fails */
  onImportError?: (error: Error) => void
  /** Whether to show auto-mount functionality (content script context) */
  autoMount?: boolean
}

export const ParseTabCore = ({
  onImportSuccess,
  onParseError,
  onImportError,
  autoMount = false,
  ...
  const handleFetchDanmaku = () => {
    ...
    mutation.mutate(
      ...
      {
        onSuccess: (data) => {
          const successMessage = autoMount
            ? t('searchPage.parse.importAndMountSuccess')
            : t('searchPage.parse.importSuccess')
          toast.success(successMessage)
          onImportSuccess?.(data)
        },
        onError: (error) => {
          const errorMessage = t('danmaku.alert.fetchError', {
  ...
  useEffect(() => {
    if (query.error) {
      ...
      toast.error(errorMessage)
      onParseError?.(query.error as Error)
    }
  }, [query.error, onParseError])
      ...
      {query.data && (
        <>
          ...
          <Box p={2}>
            ...
            >
              {autoMount
                ? t('searchPage.parse.importAndMount')
