...
用于关联[自动模式](/docs/auto-mode)。

## 网址（URL模式）

相当于一个白名单，只有在这些网址上插件才会生效。

准确地说，装填配置对应的网址不是单一的地址，而是一个[**模式**](https://developer.mozilla.org/en-US/docs/Web/API/URLPattern/URLPattern)，这个模式可以匹配多个网址。
...
同时模式必须是 HTTPS 的地址，不支持非 HTTPS 地址。
:::

最宽松的模式是 `https://*/*`，这个模式可以匹配所有的 HTTPS 地址。
