...
当弹幕本身带有渐变色时（如 Bilibili 高级弹幕），会额外拥有以下类名：

| 类名                          | 描述                 |
| :---------------------------- | :------------------- |
| `.da-danmaku-gradient`        | 普通渐变弹幕。       |
| `.da-danmaku-gradient-stroke` | 带有描边的渐变弹幕。 |
...
| `--gradient-start`  | 渐变起始颜色               | `#ff00ff`      |
| `--gradient-end`    | 渐变结束颜色               | `#00ffff`      |
| `--gradient-stroke` | 描边宽度（仅用于描边渐变） | `2px` 或 `0px` |
...
对于自带渐变色的弹幕，你可以覆盖其渐变色：

```css
.da-danmaku-gradient,
.da-danmaku-gradient-stroke {
  /* 使用 !important 覆盖内联样式 */
  --gradient-start: #ffc745ff !important;
  --gradient-end: #ff4027ff !important;
}
```
